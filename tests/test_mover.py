...
def test_intialise_mover():
    ...
    rho = 1
    cell = Mover(curve=curve, deriv=curve_deriv, centroid=centroid, density=rho)
    ...
    assert isinstance(cell.velocity, float) or isinstance(cell.velocity, complex)
    assert isinstance(cell.centroid, float) or isinstance(cell.centroid, complex)
